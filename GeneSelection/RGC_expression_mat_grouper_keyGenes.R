...
RGC_cluster_hist[['Full']] <- ExpressionTableMaker(expMatrix, gene_subset)
...
for (clust in clustIDs) {
  ...
  RGC_cluster_hist[[clust]] <- ExpressionTableMaker(RGCmat, gene_subset)
...
save(RGC_cluster_hist, file="RGC_cluster_ExpressionFrequencies_keyGenes.RData")
# Save summary stats
save(RGC_cluster_expression, RGC_cluster_expressing, RGC_cluster_SD, RGC_cluster_median, file="RGC_cluster_ExpressionMats_keyGenes.RData")
...
saveWorkbook(wb, file = "RGC_cluster_hist.xlsx", overwrite = TRUE)
