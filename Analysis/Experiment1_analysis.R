...
path <- '/home/sam/scRNAseq/Xenium/ClassVsSubclass/Experiment1/'
...
combined_data <- combined_data %>%
  extract(col = experiment_id,
          into = c("classweight", "replicateID"),
          regex = ".*_classweight_(\\d+\\.?\\d*)_result_replicate(\\d+).*",
...
analyze_trends <- function(data, score_column) {
  # Summarize data
  summary_data <- data %>%
    select(Cluster, classweight, replicateID, !!sym(score_column)) %>%
    group_by(Cluster, classweight) %>%
...
plot_trends <- function(score_df, metric = "F1") {
  ...
  for (cluster in unique(score_df$Cluster)) {
    ...
    # Data for classweight = 0
    zero_data <- cluster_data %>% filter(classweight == 0)
    
    # Create the plot
    plot <- cluster_data %>%
      filter(classweight != 0) %>%
      ggplot(aes(x = classweight, y = mean_score)) +
...
SupFig8.png <- ggplot(combined_tukey_df, aes(x=cells, y = mean_diff, shape = Interpretation, color = classweight)) +
  geom_point() +
  theme_minimal() +
  scale_x_log10() +
  facet_wrap(~metric)
...
SupFig8.png
ggsave('/home/sam/Poleg-Polsky/ICML/Figures/SupFig8.pdf', SupFig8.png, device = "pdf", width = 15, height = 10)
