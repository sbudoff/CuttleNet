...
clusters <- read.csv("MouseAC_clusterfile.txt", head = TRUE, sep="\t",skip=1, col.names = c("ID", "UMAP1", "UMAP2", "Cluster"))
expMatrix <- read.csv("MouseAC_gene_expression_matrix.csv", head = TRUE, sep="\t")
...
# ACexpMatrix <- expMatrix[GOIs,]
# ACclusters <- clusters
# save(ACexpMatrix, ACclusters, file = "/home/sam/scRNAseq/Expresion_80percent_Split/AC.RData")
...
save(cluster_hist, file="AC_cluster_ExpressionFrequencies_allGenes.RData")
# Save summary stats
save(cluster_expression, cluster_expressing, cluster_SD, cluster_median, file="AC_cluster_ExpressionMats_allGenes.RData")
...
saveWorkbook(wb, file = "AC_cluster_hist_allGenes.xlsx", overwrite = TRUE)
